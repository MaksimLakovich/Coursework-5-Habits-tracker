...
1) Команда ***/start***:
   - `async def start_command()` - обработчик команды */start* в Телеграм-боте, которая вызывает эндпоинт *TelegramConnectAPIView* (привязка в приложении Telegram-аккаунта пользователя к его Telegram-профилю).
