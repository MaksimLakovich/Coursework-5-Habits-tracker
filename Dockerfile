...
# 2) apt-get install -y ... - устанавливает системные пакеты (-y означает отвечать "ДА" на всё):
...
# 3) rm -rf /var/lib/apt/lists/* - чистит кэш apt, иначе образ будет на сотни МБ тяжелее.
...
WORKDIR /habits_tracker_project

# Копируем pyproject.toml и poetry.lock (чтобы зависимости кэшировались)
COPY pyproject.toml poetry.lock* /habits_tracker_project/
...
# 2) pip install poetry - ставит менеджер зависимостей Poetry.
# 3) poetry config virtualenvs.create false - отключает виртуальные окружения (venv),
#    потому что в Docker у нас и так изоляция (каждый контейнер = отдельное окружение).
# 4) poetry install --no-root --only main - ставит только основные зависимости (без dev-зависимостей).
...
COPY . /habits_tracker_project

# Открываем порт для приложения
EXPOSE 8000

## Команда по умолчанию (запуск Django-сервера разработки)
## Эту строку в Dockerfile можно убрать, так как запуск сервера будет описан в docker-compose.yaml
